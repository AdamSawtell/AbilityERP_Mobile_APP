-- =============================================================================
-- SAW019 — Optional nightly batch scheduler (catch-up while host idle)
-- Scheduler UU: 19a0190a-c0d4-4f01-8e15-000000000001
-- Schedule: core "1 Day" UU 5a06dc76-704e-40f5-bfcf-724829332c50 (fail closed if missing)
-- =============================================================================
...
DO $$
DECLARE
  v_sched_uu CONSTANT TEXT := '19a0190a-c0d4-4f01-8e15-000000000001';
...
BEGIN
  ...
  IF v_process_id IS NULL THEN
    RAISE EXCEPTION 'SAW019: batch process missing — run 05 first';
  END IF;
  ...
  IF v_schedule_id IS NULL THEN
    RAISE EXCEPTION 'SAW019: AD_Schedule "1 Day" missing (UU 5a06dc76-704e-40f5-bfcf-724829332c50)';
  END IF;
  ...
  IF v_sched_id IS NULL THEN
    SELECT ad_scheduler_id INTO v_sched_id FROM ad_scheduler WHERE name = 'Invoice Capture Nightly Batch' LIMIT 1;
  END IF;
  ...
  IF v_sched_id IS NULL THEN
    INSERT INTO ad_scheduler (
      ...
      NOW(), 100, NOW(), 100,
      'Invoice Capture Nightly Batch',
      'Catch-up sweep of eligible Invoice Capture rows (same service as Process Selected Invoice)',
  ...
  ELSE
    UPDATE ad_scheduler SET
      name = 'Invoice Capture Nightly Batch',
  ...
  RAISE NOTICE 'SAW019 scheduler ready (process=%)', v_process_id;
END $$;
