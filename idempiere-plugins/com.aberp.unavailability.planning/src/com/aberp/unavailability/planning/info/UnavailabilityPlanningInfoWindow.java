package com.aberp.unavailability.planning.info;
 ...
 * </ul>
 */
public class UnavailabilityPlanningInfoWindow extends InfoWindow {

	public static final String INFO_WINDOW_UU = "21a021iw-c0d4-4f01-8e15-000000000001";
	...
	public UnavailabilityPlanningInfoWindow(int windowNo, String tableName, String keyColumn, String queryValue,
	...
	/**
	 * ReQuery path: InfoPanel.onUserQuery  to  validateParameters  to  query.
	...
	protected String getSQLWhere() {
		...
		return withPlanningDatesCleared(snap, (s, e) -> {
			...
			if (s == null || e == null) {
				log.warning("UnavailabilityPlanning getSQLWhere missing dates start=" + s + " end=" + e + " — blocking unfiltered grid");
				where = appendAnd(where, "1=0");
			} else {
				where = appendAnd(where, "ou.EndDate::date >= DATE '" + toSqlDate(s) + "'");
				where = appendAnd(where, "ou.StartDate::date <= DATE '" + toSqlDate(e) + "'");
			}
			log.warning("UnavailabilityPlanning getSQLWhere start=" + s + " end=" + e + " where=" + where);
			return where;
		});
	}
	...
	private Timestamp readPlanningDate(String columnName) {
		...
		for (WEditor ed : allEditorsFor(columnName)) {
			...
			if (ts == null) {
				ts = timestampFromComponent(ed != null ? ed.getComponent() : null);
			}
			if (ts != null) {
				return ts;
			}
		}
		// Fallback: criteria Dateboxes in panel order (ZK sometimes leaves WEditor value null).
		ts = readPlanningDateFromPanel(columnName);
		if (ts != null) {
			return ts;
		}
		log.warning("UnavailabilityPlanning missing date value for " + columnName);
		return null;
	}

	private Timestamp readPlanningDateFromPanel(String columnName) {
		java.util.List<org.zkoss.zul.Datebox> boxes = new java.util.ArrayList<>();
		collectDateboxes(parameterGrid, boxes);
		if (boxes.isEmpty()) {
			collectDateboxes(this, boxes);
		}
		int idx = "AbERP_PlanningStart".equalsIgnoreCase(columnName) ? 0
				: "AbERP_PlanningEnd".equalsIgnoreCase(columnName) ? 1 : -1;
		if (idx < 0 || idx >= boxes.size()) {
			return null;
		}
		return timestampFromComponent(boxes.get(idx));
	}

	private static void collectDateboxes(Component root, java.util.List<org.zkoss.zul.Datebox> out) {
		if (root == null || out == null) {
			return;
		}
		if (root instanceof org.zkoss.zul.Datebox) {
			out.add((org.zkoss.zul.Datebox) root);
		}
		java.util.List<Component> children = root.getChildren();
		if (children != null) {
			for (Component child : children) {
				collectDateboxes(child, out);
			}
		}
	}

	private static Timestamp timestampFromComponent(Component c) {
		if (!(c instanceof org.zkoss.zul.Datebox)) {
			return null;
		}
		org.zkoss.zul.Datebox db = (org.zkoss.zul.Datebox) c;
		Timestamp ts = toTimestamp(db.getValue());
		if (ts == null) {
	...
	private static final String SQL_SUPPORT_LOC_NAMES =
			"aberp_up_primary_support_location(u.AD_User_ID)";
	...
	protected void renderItems() {
		...
		Events.echoEvent("onUnavailabilityPlanningColour", this, null);
	}
	...
	private void ensureColourListeners() {
		...
		addEventListener("onUnavailabilityPlanningColour", event -> colourApproverStatusCells());
	...
	private void colourApproverStatusCellsJs() {
		String js = "setTimeout(function(){(function(){"
				...
				+ "  return (jq(this).text()||'').indexOf('Unavailability Planning summary')>=0;"
				+ "}).last();"
	...
	private void ensureSummaryBanner(North north) {
		...
		periodLabel = new Label("Unavailability Planning summary  -  set Planning Start and Planning End, then Search.");
	...
	private void refreshSummaryBanner() {
		...
		Timestamp end = readPlanningDate("AbERP_PlanningEnd");
		if (start == null || end == null) {
			periodLabel.setValue("Unavailability Planning summary  -  set Planning Start and Planning End, then Search.");
			setStatusCounts(null);
			typeLabel.setValue("Day lines:  - ");
			return;
		}
		...
		try {
			...
			String period = df.format(new Date(start.getTime())) + "  to  " + df.format(new Date(end.getTime()));
			...
			typeLabel.setValue("Day lines: " + (Util.isEmpty(byType, true) ? " - " : byType));
		} catch (Exception ex) {
			periodLabel.setValue("Unavailability Planning summary unavailable: " + ex.getMessage());
			setStatusCounts(null);
			typeLabel.setValue("Day lines:  - ");
		}
	}
	...
	private void exportCsv() {
		...
		try {
			...
			setStatusLine("Exported " + rows + " unavailability row(s) to " + name, false);
		} catch (Exception ex) {
