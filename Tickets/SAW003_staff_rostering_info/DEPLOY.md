...
**Current bundle:** **`1.1.0.2026071511`**
...
./deploy.sh
# build JAR 1.1.0.2026071511 → SQL 01→24→04 → restart → wait WebUI 200
...
cd idempiere-plugins/com.aberp.rostering.staffinfo
chmod +x build.sh
# Confirm VERSION=1.1.0.2026071511 in build.sh + META-INF/MANIFEST.MF
./build.sh
JAR=com.aberp.rostering.staffinfo_1.1.0.2026071511.jar
...
echo "com.aberp.rostering.staffinfo,1.1.0.2026071511,plugins/$JAR,4,true" | sudo tee -a "$B" >/dev/null
...
`/opt/idempiere-server/plugins/com.aberp.rostering.staffinfo_1.1.0.2026071511.jar` — scp + same install steps above.
...
| Version | **`1.1.0.2026071511`** (`build.sh` / `deploy.sh` / `META-INF/MANIFEST.MF`) |
...
Prefer host `./build.sh` so the JAR matches MANIFEST. Known-good binary: **≥ ~40 KB** (≈57 KB for `1511`). ~29 KB with a version stamp is a **stale** banner-only build.
...
- Credential box is a sibling **below** `parameterGrid` (not inside a grid row). Show Unmatched expands **only this Info Window’s** North (by uuid) — never `jq('.z-north').get(0)` (that inflates the parent Shift window and leaves a white gap after close). Restore North height on close / hide unmatched (`1511`).
...
3. Leave hidden unless **Show Unavailable Staff**; overlapping roster excluded by default via **Employee Not Rostered at this Time** (untick to include); needs mismatches hidden unless **Show Unmatched Staff**.    
4. Lean grid (no BP Name / Status / Business Partner / Agency Staff columns).  
5. Related Info tabs; contact pick fills BP.

**Show Unmatched + credential AND (JAR `1511`):**
...
|-----|------|--------|-------|
| Staging EC2 | `ec2-54-206-120-32…:8080` | `1.1.0.2026071511` | SSH key `AbilityERP_Development_Keypair_Shared.pem` |
| HCO Test | `http://13.210.248.141/webui/` | `1.1.0.2026071511` | SSH `ubuntu@13.210.248.141` · `HCObusiness.pem` (old IP `32.236.127.117` retired) |
...
Refresh packs when shipping a client zip: JAR **`1511`** (≥ ~40 KB) + SQL through **`24`**. Prefer host `./deploy.sh` / JAR-only steps over stale packs.

## Agent prompt (copy)

> Deploy SAW003 per `Tickets/SAW003_staff_rostering_info/DEPLOY.md`. Prefer JAR-only update to `1.1.0.2026071511` if SQL is already applied. Run WebUI smoke: Show Unmatched → two-column Find + Select (AND); report pass/fail.
