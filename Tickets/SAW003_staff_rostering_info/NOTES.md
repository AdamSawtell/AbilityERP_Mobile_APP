...
- Bundle: `com.aberp.rostering.staffinfo` ΓÇö version **`1.1.0.2026071227`** (`build.sh` / `deploy.sh` / MANIFEST / Downloads packs).
...
1. **Original problem:** Shift ΓåÆ Employee staff search was slow/heavy (fat FROM joins) and hard to filter leave / overlap / credential needs.
2. **Rewrite (SAW003):** Lean User+BP query, Java EXISTS filters, Related Info, clearer UX.
3. **HCO follow-ups:** ZK **non-negative only** popup (Multi Select `-1` + missing `c_bpartner_staff_id` on CredentialAssignment) ΓÇö fixed in SQL `21`ΓÇô`23` + Java sanitize / schema guard.
...
| Java `1222` | Show Unmatched ΓåÆ credential multi-select Listbox (AND); empty = unmatched pool only |
...
**Empty results Γëá non-negative popup.** No staff matching needs ΓåÆ `0 Rows found` (tick **Show Unmatched Staff**). Popup is ZK `-1` Intbox validation.
...
| Info Window UU | `2b4ab146-0809-47c6-96f3-8b841d60a6bf` | Local `AD_InfoWindow_ID` = **1000034** (Γëá seed `1000027`) |
| OLD IW (do not use) | UU `d0a2aeb5-ΓÇª` local `1000042` | Leave alone |
| Shift (Rostered) window | UU `7c269a7e-ΓÇª` local `1000082` | Employee tab `1000149` |
| Employee Search field | `AbERP_User_Contact_ID` ΓåÆ ref `1000215` | Already points at IW `1000034` |
| Admin IW access | Admin + AbilityERP Admin + Rostering (+TL) | Already granted |
| JAR | `ΓÇª_1.1.0.2026071221.jar` | Unmatched credential multi-select |
...
| Area | Status |
|------|--------|
| SQL `01`ΓÇô`24`ΓåÆ`04` | Applied on HCO (through perf / non-negative hotfixes) |
...
| Unmatched credential AND multi-select | JAR `1227` (staging EC2); redeploy to HCO when ready |
...
| Full smoke from Shift ΓåÆ Employee Search | Re-test ReQuery with All/Any after logout/in |
