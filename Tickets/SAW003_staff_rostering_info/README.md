...
**→ [`DEPLOY.md`](DEPLOY.md)** — full `./deploy.sh` or **JAR-only** update to `1511`, SQL order, smoke, pitfalls.
...
Rewrite **Employee (User) / Agency Staff Rostering Info** used on **Shift (Rostered) → Employee**: lean query, leave + overlapping-roster filters (independent ticks), needs-match in Java, UX wildcards, Related Info, org/BP callouts, decluttered result grid, **Show Unmatched** credential multi-select (AND) with Find + two-column layout.
