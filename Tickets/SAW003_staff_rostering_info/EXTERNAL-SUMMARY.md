...
**Status:** Ready for client install (JAR **1.1.0.2026071511** on staging and HCO Test)  
...
| **OSGi plugin** | `com.aberp.rostering.staffinfo` `1.1.0.2026071511` | Java Info + callout (restart required) |
| **Process** | *(none new)* | — |
| **Menu** | *(none new)* | — |
...
- When opened from a shift: hide approved leave overlapping the shift window by default; hide overlapping roster by default via **Employee Not Rostered at this Time**
...
- **Show Unavailable Staff** tick: when on, include staff on approved leave (leave-only — overlapping roster uses the tick above)
...
3. Confirm **Staff Name**, Employee, Agency Staff criteria and ticks: **Show Unmatched Staff**, **Employee Not Rostered at this Time** (default on), **Show Unavailable Staff**.
4. Search by name without `%`.
5. Confirm banner shows shift context (not “No shift in context”).
6. Confirm leave hidden unless Show Unavailable is ticked; overlapping roster excluded unless Not Rostered is unticked; needs mismatches hidden unless Show Unmatched is ticked.
...
- JAR must be **`1.1.0.2026071511`** and **≥ ~40 KB**. A ~29 KB file with an older version string is a **stale build** (banner only — missing ticks / credential list).
