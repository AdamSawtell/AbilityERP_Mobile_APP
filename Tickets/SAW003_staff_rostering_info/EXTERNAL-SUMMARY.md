...
**Status:** Ready for client install (JAR **1.1.0.2026071513** on staging and HCO Test)  
...
| **OSGi plugin** | `com.aberp.rostering.staffinfo` `1.1.0.2026071513` | Java Info + callout (restart required) |
| **Process** | *(none new)* | — |
| **Menu** | *(none new)* | — |
...
- JAR must be **`1.1.0.2026071513`** and **≥ ~40 KB**. A ~29 KB file with an older version string is a **stale build** (banner only — missing ticks / credential list).
