...
- **SQL 41 (2026-07-19):** sixth category **Support Location** (`L`). Live population + attribute KPIs via `aberp_compliance_location_kpi` / `_bucket` ColumnSQL (no view recreate). Findings nest + DisplayLogic `@ActiveSupportLocations@>-1`. Engine writes `L` snapshot on Refresh (population only until rules exist). Browser smoke: Active **35**, Vacant **7**, SDA **13**, Wheelchair **8**, Bushfire **17**, Meets **34**, Status **G**, no SQL modal.
